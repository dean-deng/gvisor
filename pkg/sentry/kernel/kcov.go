...
import (
  ...
  // TODO(deandeng): update comment
	// Some explanation: with coverage enabled, bazel uses the go cover
	// tool manually to generate instrumented files. It also injects a hook
	// that registers all coverage data with the coverdata package, which
	// is exactly what we want to expose coverage to userspace.
...
func (rw *kcovReadWriter) ReadToBlocks(dsts safemem.BlockSeq) (uint64, error) {
  ...
  start := rw.fr.Start + rw.off
	...
	bs, err := rw.mf.MapInternal(platform.FileRange{start, end}, usermem.Read)
	if err != nil {
		return 0, err
	}
...
func (rw *kcovReadWriter) WriteFromBlocks(srcs safemem.BlockSeq) (uint64, error) {
  ...
  if wend := start + srcs.NumBytes(); wend < end {
	...
	bs, err := rw.mf.MapInternal(platform.FileRange{start, end}, usermem.Write)
	if err != nil {
		return 0, err
	}
